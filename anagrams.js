...
function is_anagram_of(str1, str2) {
	...
	// loop through the second word (you are only comparing the first letter of the first word)
	for(var j = 0; j < str2.length; j++) {
		// if the letters are equal and they are the only letters in the strings return true
		if(str1[0] === str2[j] && str1.length === 1) {
			return true
		}
		// if the letters are equal (and aren't the only letters) then take out the matching 
		// letters and run the algorithm on the remaining strings
...
// anagrams function returns a js object with keys that are the first instance of the word
// and values that are the number of anagrams 
